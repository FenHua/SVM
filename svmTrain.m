...
switch svmType
    case 'svc_c',
        ...
        %This MATLAB function returns a vector x that minimizes 1/2*x'*H*x?+?f'*x.
    case 'svc_nu',
        nu=p1;
        n=length(Y);
        H=(Y'*Y).*kernel(ker,X,X);
    ...
    case 'svr_nu',
        ...
        ub=C*ones(2*n,1);
        a0=zeros(2*n,1);
        [a,fval,exitflag,output,lambda]=quadprog(H,f,A,b,Aeq,lb,ub,a0,options);
        a=a(1:n)-a(n+1:end);
    otherwise,
end
exitflag
